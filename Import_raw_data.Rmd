...
pacman::p_load(tidyverse, skimr, here)
```
...
dat <-
   read_csv(here("data", "lesion_counts.csv")) %>%
...
dat <-
   ...
   mutate(
      SpEv = case_when(
         ...
         SpEv == "Horsham_1" ~ "Horsham Dryland 1",
         SpEv == "Horsham_2" ~ "Horsham Dryland 2",
         SpEv == "Curyo_1" ~ "Curyo Dryland 1"
      )
   )
...
write_csv(x = dat, path = here("cache", "summarised_lesion_data.csv"))
