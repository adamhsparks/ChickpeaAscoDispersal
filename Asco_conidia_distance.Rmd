...
BLUP_rep.site$site <- data.frame(do.call(rbind, strsplit(row.names(BLUP_rep.site), split = ":")))[,1]
BLUP_rep.site$rep <- data.frame(do.call(rbind, strsplit(row.names(BLUP_rep.site), split = ":")))[,2]
BLUP_rep.site[,2:5] <- BLUP_rep.site[,1] + BLUP_rep.site[,2:5]
p1_dat <- reshape2::melt(BLUP_rep.site[c(3,5,6,2,4,1),])

names(p1_dat) <- c("rep","site", "distance","lesions")
