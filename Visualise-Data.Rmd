...
theme_set(theme_pubclean(base_size = 15))
```
...
Note that this does not indicate degrees direction.
It is just the 10 transects on a polar coordinate.

```{r polar_coord_transects, message = FALSE}
ggplot(dat, aes(x = transect, y = dist)) +
   geom_count(aes(size = mean_count_pot, colour = SpEv), alpha = 0.5) +
   scale_colour_usq("bright") +
   ylab("Distance (m)") +
   xlab("Transect") +
   coord_polar() +
...
This should work once we have the direction available as degrees for each transect.

```{r polar_coord_transects, message = FALSE, eval=FALSE}
ggplot(dat, aes(x = mean_count_pot, y = dist, colour = direction)) +
   geom_count(aes(size = mean_count_pot)) +
